...
exports.bulkCreateStates = bulkCreateStates;
...
async function createState(request, response) {
    ...
    const validationRules = [
        (0, express_validator_1.body)('name').notEmpty().withMessage('State name is required').bail().isString().trim().isLength({ min: 2 }).withMessage('State name too short'),
        (0, express_validator_1.body)('code').optional({ nullable: true }).isString().trim().isLength({ min: 1 }).withMessage('code must be a string'),
        (0, express_validator_1.body)('countryId').notEmpty().withMessage('countryId is required').bail().isInt().withMessage('countryId must be an integer')
    ];
    ...
    try {
        ...
        if (existing)
            return response.status(400).json({ message: 'State already exists for this country' });
...
async function listStates(request, response) {
    const countryId = request.query.country_id ? parseInt(request.query.country_id, 10) : undefined;
    try {
...
async function getState(request, response) {
    ...
    if (!id)
        return response.status(400).json({ message: 'State id required' });
    try {
...
async function deleteState(request, response) {
    const id = parseInt(request.params.id, 10);
    if (!id)
        return response.status(400).json({ message: 'State id required' });
    try {
 ...
 */
async function bulkCreateStates(request, response) {
    const admin_id = request.admin?.adminId;
    if (!admin_id)
        return response.status(403).json({ message: 'Unauthorized User' });
    // validate request using express-validator
    const validationRules = [
        (0, express_validator_1.body)().isArray({ min: 1 }).withMessage('Request body must be a non-empty array'),
        // each item should be an object that contains at least one of state/name/alias
        (0, express_validator_1.body)('*').custom((item) => {
            if (typeof item !== 'object' || item === null)
                throw new Error('Each item must be an object');
            if (!item.state && !item.name && !item.alias)
                throw new Error('Each item must have one of: state, name or alias');
            return true;
        }),
        (0, express_validator_1.body)('*.lgas').optional().isArray().withMessage('lgas must be an array of strings'),
        (0, express_validator_1.body)('*.lgas.*').optional().isString().trim(),
        (0, express_validator_1.body)('*.state').optional().isString().trim(),
        (0, express_validator_1.body)('*.name').optional().isString().trim(),
        (0, express_validator_1.body)('*.alias').optional().isString().trim(),
        // allow optional countryId as top-level field (if client sends an object wrapper)
        (0, express_validator_1.body)('countryId').optional().isInt().withMessage('countryId must be an integer'),
    ];
    await Promise.all(validationRules.map(rule => rule.run(request)));
    const errors = (0, express_validator_1.validationResult)(request);
    if (!errors.isEmpty()) {
    ...
    const countryIdFromBody = request.body.countryId ?? undefined;
    const countryIdFromQuery = request.query.country_id ? parseInt(request.query.country_id, 10) : undefined;
    let countryId = countryIdFromBody || countryIdFromQuery;
    try {
        if (!countryId) {
        ...
        if (!countryId)
            return response.status(400).json({ message: 'countryId is required either in body, query or a Nigeria country record must exist' });
        const results = [];
        for (const item of payload) {
            const stateName = (item.state || item.name || item.state_name || item.alias);
