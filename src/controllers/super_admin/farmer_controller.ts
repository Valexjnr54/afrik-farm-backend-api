...
export async function createFarmer(request: Request, response: Response) {
    ...
    const rules = [
        ...
        body('countryId').notEmpty().withMessage('Country Id is required').bail().isInt().withMessage('Country Id must be an integer'),
        body('stateId').notEmpty().withMessage('State Id is required').bail().isInt().withMessage('State Id must be an integer'),
        body('lgaId').notEmpty().withMessage('LGA Id is required').bail().isInt().withMessage('LGA Id must be an integer'),
    ];
...
export async function profile_imageUpload(request: Request, response: Response) {
    const lga_admin_id = request.admin?.adminId;
...
// Super-admin helper endpoints
// Returns all farmers who have paid
export async function getPaidFarmers(request: Request, response: Response) {
    ...
    try {
        ...
        const paidFarmers = await prisma.farmer.findMany({
            where: { has_paid: true },
            orderBy: { createdAt: 'desc' },
            select: {
                id: true,
        ...
        return response.status(200).json({ message: 'Paid farmers fetched', count: paidFarmers.length, data: paidFarmers });
    } catch (error) {
        console.error('getPaidFarmers error', error);
        return response.status(500).json({ message: 'Internal Server Error' });
    }
}

// Returns all farmers who have NOT paid
export async function getUnpaidFarmers(request: Request, response: Response) {
    const admin_id = (request as any).admin?.adminId;
    if (!admin_id) return response.status(403).json({ message: 'Unauthorized User' });
    ...
    try {
        ...
        const unpaidFarmers = await prisma.farmer.findMany({
        ...
        return response.status(200).json({ message: 'Unpaid farmers fetched', count: unpaidFarmers.length, data: unpaidFarmers });
    } catch (error) {
        console.error('getUnpaidFarmers error', error);
        return response.status(500).json({ message: 'Internal Server Error' });
    }
}
